...
const UserRegister = () => {
  ...
  const hrUser = JSON.parse(sessionStorage.getItem("active-hr"));
  ...
  useEffect(() => {
    if (document.URL.indexOf("company") != -1) {
      user.role = "Company";
    } else if (document.URL.indexOf("hr") != -1) {
      user.role = "HR";
      user.companyId = company.company.id;
    } else if (document.URL.indexOf("employee") != -1) {
      user.role = "Employee";
      user.companyId = hrUser.company.id;
    }
  }, []);
